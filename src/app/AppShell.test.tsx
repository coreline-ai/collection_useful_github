...
describe('AppShell', () => {
  ...
  it('renders github feature by default', async () => {
    render(<AppShell />)
  ...
  it('switches to unified-search section and hides github board', async () => {
    render(<AppShell />)

    fireEvent.click(screen.getByRole('tab', { name: '통합검색' }))

    expect(screen.getByRole('tab', { name: '통합검색' })).toHaveAttribute('aria-selected', 'true')
    expect(await screen.findByLabelText('통합 검색어')).toBeInTheDocument()
    expect(screen.queryByLabelText('GitHub 저장소 URL')).not.toBeInTheDocument()
  })

  it('keeps unified-search input state across top section switches', async () => {
    render(<AppShell />)

    fireEvent.click(screen.getByRole('tab', { name: '통합검색' }))
    fireEvent.change(await screen.findByLabelText('통합 검색어'), { target: { value: 'react' } })

    fireEvent.click(screen.getByRole('tab', { name: '깃허브' }))
    fireEvent.click(screen.getByRole('tab', { name: '통합검색' }))

    expect(await screen.findByLabelText('통합 검색어')).toHaveValue('react')
  })

  it('switches to bookmark board and hides github/search UI', async () => {
    render(<AppShell />)
  ...
  it('switches to youtube board with youtube input', async () => {
    render(<AppShell />)
  ...
  it('restores selected section from localStorage', async () => {
