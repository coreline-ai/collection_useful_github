import {
  claimNextYoutubeSummaryJob,
  getYoutubeSummaryStaleLockMs,
  markYoutubeSummaryJobFailed,
  markYoutubeSummaryJobSucceeded,
  recoverStuckYoutubeSummaryJobs,
} from './youtubeSummaryQueue.js'

const DEFAULT_WORKER_POLL_INTERVAL_MS = Number(process.env.YOUTUBE_SUMMARY_WORKER_POLL_INTERVAL_MS || 1500)
const DEFAULT_WORKER_STALE_LOCK_MS = getYoutubeSummaryStaleLockMs()
const DEFAULT_WORKER_RECOVERY_INTERVAL_MS = Number(process.env.YOUTUBE_SUMMARY_RECOVERY_INTERVAL_MS || 30 * 1000)

export const computeYoutubeSummaryRetryDelayMs = (attemptCount) => {
...
export const classifyYoutubeSummaryError = (error) => {
...
export const startYoutubeSummaryWorker = ({
  workerId = `yt-summary-worker-${process.pid}`,
  ...
  if (typeof processJob !== 'function') {
    throw new Error('processJob callback is required to start youtube summary worker')
  }
  ...
  const processNext = async () => {
    ...
    try {
      ...
      if (now - lastRecoveryAt >= safeRecoveryInterval) {
        lastRecoveryAt = now
        await recoverStuckYoutubeSummaryJobs({ staleMs: staleLockMs })
      }

      const job = await claimNextYoutubeSummaryJob({ workerId })
      if (!job) {
        return
      }
      ...
      try {
        ...
        await markYoutubeSummaryJobSucceeded({
          jobId: job.id,
          summaryText: result?.summaryText || result?.resultSummary || '',
        })
      } catch (error) {
        const { retryable, errorCode } = classifyYoutubeSummaryError(error)
        const errorMessage = error instanceof Error ? error.message : '요약 워커 실행에 실패했습니다.'
        const nextRunAt = new Date(Date.now() + computeYoutubeSummaryRetryDelayMs(job.attemptCount)).toISOString()

        await markYoutubeSummaryJobFailed({
          jobId: job.id,
          retryable,
          errorCode,
          errorMessage,
