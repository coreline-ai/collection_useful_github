...
const buildBookmarkSummaryPrompt = (metadata) => {
  const title = String(metadata?.title || '').trim()
  const excerpt = String(metadata?.excerpt || '').trim()
  const domain = String(metadata?.domain || '').trim()
  const normalizedUrl = String(metadata?.normalizedUrl || metadata?.bookmarkId || '').trim()

  return [
    '아래 북마크 정보를 바탕으로 한국어 요약을 작성해 주세요.',
    '요구사항:',
    '1) 정확히 3문장',
...
const summarizeBookmarkWithGlm = async (metadata, options = {}) => {
  ...
  const prompt = buildBookmarkSummaryPrompt(metadata)
    ...
    `${baseUrl}/chat/completions`,
    {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You summarize web bookmarks in Korean for productivity dashboards.',
          },
          {
            role: 'user',
            content: prompt,
...
export const resolveBookmarkSummaryConfig = (env = process.env) => {
  const timeoutSeconds = Number(env.BOOKMARK_SUMMARY_TIMEOUT_SECONDS || 30)
  const timeoutMs = Number.isFinite(timeoutSeconds) && timeoutSeconds > 0 ? Math.floor(timeoutSeconds * 1000) : 30_000

  return {
    summaryEnabled: parseBoolean(env.BOOKMARK_SUMMARY_ENABLED, true),
    summaryProvider: String(env.BOOKMARK_SUMMARY_PROVIDER || 'glm').trim().toLowerCase(),
    timeoutMs,
    glm: {
...
export const generateBookmarkSummaryState = async ({
  metadata,
  currentCard,
  force = false,
  config = resolveBookmarkSummaryConfig(),
}) => {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const summaryText = await summarizeBookmarkWithGlm(metadata, {
